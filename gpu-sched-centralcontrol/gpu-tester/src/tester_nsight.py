...
lib =cdll.LoadLibrary(os.path.abspath("../centralcontrol/libgeek.so")) 
...
class SchedulerTester():
    ...
    def infer(self):
        ...
        if self.control and (self.idx == 1 or self.idx == 2):
            try:
                controlName = c_char_p()
                controlName.value = b"current_process"
                lib.sendToCentral(controlName, "1")
            ...
            try:
                controlName = c_char_p()
                controlName.value = b"current_process"
                lib.sendToCentral(controlName, "0")
            except Exception as e: print(e)
