...
memcached_st *memc;
memcached_return rc;
int init_connection = -1;

/***********Communication to Central controller start***************/
void sendToCentral(const char* controlName, const char *value)
{
    //connect to server 
    if (init_connection == -1)
    {
        ...
        {
          fprintf(stderr,"Hook couldn't add server: %s\n",memcached_strerror(memc, rc));
          return;
        }
        init_connection = 1;
    }
...
void requestFromCentral(const char *controlName, char **result, size_t *resultSize)
{
    //connect to server 
    if (init_connection == -1)
    {
		...
		void** kernelParams, void** extra)
{
    ...
    if (get_id() == 0) {
        ...
        free(timestamp);
#endif 
        const char* controlName = "current_process";
        char *result;
        size_t resultSize;
        requestFromCentral(controlName, &result, &resultSize);
        while(strcmp(result, "1") == 0) {
#ifdef _VERBOSE_WENQING
	        char *timestamp = printUTCTime();
