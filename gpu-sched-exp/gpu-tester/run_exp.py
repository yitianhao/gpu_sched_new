...
def output(sync_list_p0):
    for sync in sync_list_p0:
        print("--------------------------------------------")
        print(f"control + sync, sync = {sync}")
        filename_model0 = f"logs/model_A_sync{sync}.log"
        filename_model1 = f"logs/model_B_sync{sync}.log"
        ...
        print(f"{model} {size} | avg JCT = {sum(jct) / len(jct)}")
        f.close()
...
def main():
    ...
    for sync in sync_list_p0:
        ...
        for i, model_file in enumerate(model_files):
            ...
            # logging
            logfilename = f"{model['output_file_path']}/{model['output_file_name']}_sync{sync}.log"
            ...
            if (control and controlSync):
                ...
                process_env['SYNC_KERNELS'] = str(sync)
