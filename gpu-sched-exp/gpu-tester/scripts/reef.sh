...
# model_A_name=maskrcnn_resnet50_fpn_v2
# model_A_weight=MaskRCNN_ResNet50_FPN_V2_Weights
# model_A_name=keypointrcnn_resnet50_fpn
# model_A_weight=KeypointRCNN_ResNet50_FPN_Weights
...
for index in "${!model_B_names[@]}"; do
    ...
    exp_name=${model_A_name}_batch_${model_A_batch_size}_${model_A_input_size}p_vs_${model_B_names[$index]}_batch_${model_B_batch_size}_${model_B_input_size}p

    sync=4
    out_path="../reef/${exp_name}/"
    mkdir -p $out_path
    exp_config_file=${out_path}/exp_config.json
    jq .models[0].control.queue_limit.sync=$sync ${config_file}  | \
    jq .models[1].control.queue_limit.sync=$sync | \
